...
class MySort:
    ...
    def quickSort(self, begin_id=0, end_id=0):
        ...
        pivot = self._partition(begin_id, end_id)
    ...
    def heapSort(self):
        ...
        for i in range((len(self.array) - 2) // 2, -1, -1):
            ...
            # 从最后一个非叶子节点开始，依次左heapifyDown调整
            self.heapifyDown(i, len(self.array))

        # 每次把堆顶和j对换，然后调整
        for j in range(len(self.array) - 1, 0, -1):
            # 把堆顶（即最大元素）与当前j的位置的元素对换，j即是剩余元素中的最大值应插入的位置
            self.array[j], self.array[0] = self.array[0], self.array[j]

            # todo: 从堆顶向下调整，并且只调整到j（因为j之后是已经排序好了的）
            self.heapifyDown(0, j)
